...
namespace editor::tools
{
    geometry PrimitiveGenerator::create_plane(const float x, const float z)
    {
        ...
        {
            {
                { { -half_x, 0.0f,  half_z }, engine::math::vec3::up() },
                { {  half_x, 0.0f,  half_z }, engine::math::vec3::up() },
                { {  half_x, 0.0f, -half_z }, engine::math::vec3::up() },
                { { -half_x, 0.0f, -half_z }, engine::math::vec3::up() }
            },
    ...
    geometry PrimitiveGenerator::create_box(const float x, const float y, const float z)
    {
        ...
        {
            {
                { { -half_x, -half_y,  half_z }, engine::math::vec3::front() },
                { {  half_x, -half_y,  half_z }, engine::math::vec3::front() },
                { {  half_x,  half_y,  half_z }, engine::math::vec3::front() },
                { { -half_x,  half_y,  half_z }, engine::math::vec3::front() },

                { { -half_x, -half_y, -half_z }, engine::math::vec3::back() },
                { { -half_x,  half_y, -half_z }, engine::math::vec3::back() },
                { {  half_x,  half_y, -half_z }, engine::math::vec3::back() },
                { {  half_x, -half_y, -half_z }, engine::math::vec3::back() },

                { { -half_x,  half_y,  half_z }, engine::math::vec3::left() },
                { { -half_x,  half_y, -half_z }, engine::math::vec3::left() },
                { { -half_x, -half_y, -half_z }, engine::math::vec3::left() },
                { { -half_x, -half_y,  half_z }, engine::math::vec3::left() },

                { {  half_x,  half_y,  half_z }, engine::math::vec3::right() },
                { {  half_x, -half_y,  half_z }, engine::math::vec3::right() },
                { {  half_x, -half_y, -half_z }, engine::math::vec3::right() },
                { {  half_x,  half_y, -half_z }, engine::math::vec3::right() },

                { { -half_x,  half_y, -half_z }, engine::math::vec3::up() },
                { { -half_x,  half_y,  half_z }, engine::math::vec3::up() },
                { {  half_x,  half_y,  half_z }, engine::math::vec3::up() },
                { {  half_x,  half_y, -half_z }, engine::math::vec3::up() },

                { { -half_x, -half_y, -half_z }, engine::math::vec3::down() },
                { {  half_x, -half_y, -half_z }, engine::math::vec3::down() },
                { {  half_x, -half_y,  half_z }, engine::math::vec3::down() },
                { { -half_x, -half_y,  half_z }, engine::math::vec3::down() }
            },
    ...
    primitive PrimitiveGenerator::create_sphere(const uint32_t segments, const uint32_t rings, const float radius)
    {
        ...
        const auto    phi_step = engine::math::pi / static_cast<float>(rings);
        const auto  theta_step = engine::math::pi / static_cast<float>(segments) * 2.0f;
        ...
        for (uint32_t i = 0; i <= rings; ++i)
        {
            ...
            const auto sin_phi = half_radius * engine::math::sin(phi);
            const auto cos_phi = half_radius * engine::math::cos(phi);

            for (uint32_t j = 0; j <= segments; ++j)
            {
                const auto theta = theta_step * static_cast<float>(j);

                sphere.add_vertex({ sin_phi * engine::math::cos(theta), cos_phi, sin_phi * engine::math::sin(theta) });
            }
        }
    ...
    primitive PrimitiveGenerator::create_capsule(const uint32_t segments, const uint32_t rings, const float radius, const float height)
    {
        ...
        const auto phi_step   = engine::math::pi / static_cast<float>(half_rings) / 2.0f;
        const auto theta_step = engine::math::pi / static_cast<float>(segments)   * 2.0f;
        ...
        for (uint32_t i = 0; i <= half_rings; ++i)
        {
            ...
            for (uint32_t j = 0; j <= segments; ++j)
            {
                ...
                capsule.add_vertex({ sin_phi  * engine::math::cos(theta), cos_phi + half_height, sin_phi * engine::math::sin(theta) });
            }
        }
        ...
        for (uint32_t i = 0; i <= half_rings; ++i)
        {
            ...
            for (uint32_t j = 0; j <= segments; ++j)
            {
                ...
                capsule.add_vertex({ half_radius * engine::math::cos(theta), offset, half_radius * engine::math::sin(theta) });
            }
        }
        ...
        for (uint32_t i = 0; i <= half_rings; ++i)
        {
            ...
            for (uint32_t j = 0; j <= segments; ++j)
            {
                ...
                capsule.add_vertex({ sin_phi  * engine::math::sin(theta), -half_height - cos_phi, sin_phi * engine::math::cos(theta) });
            }
        }
